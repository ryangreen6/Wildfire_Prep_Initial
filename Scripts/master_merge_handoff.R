...
defsp23 <- st_read("POL S 199RA/R Stuff/defensible_space_inspections_2023.gdb") %>% 
  janitor::clean_names()
...
defsp24_addresses <- st_read("POL S 199RA/R Stuff/parcel_data/Defensible_Space_2024_address_points.gdb") %>% 
  janitor::clean_names()
...
readysetgo <- read_csv("POL S 199RA/R Stuff/parcel_data/2024_ReadySetGo_DSP_Address_List_20240415.csv") %>% 
  janitor::clean_names() %>% 
  distinct(full_address, .keep_all = TRUE)

#just the straight address with coordinate data
latlong <- read_csv("POL S 199RA/R Stuff/parcel_data/2024_Defensible_Space_LatLong_Addr.csv") %>% 
  janitor::clean_names() %>% 
